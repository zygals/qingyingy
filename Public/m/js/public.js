...
function upload1() {
    ...
    uploader.on('uploadProgress', function (file) {
        $('#img1').after('<label class="loading"></label>');
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            $('#img1').siblings('.loading').remove();
            $('#img1').show();
            layer.msg(ret.info);
        } else {
            $('#icon').val(ret.data.url);
            $("#img1").attr('onload', 'setImgWH(this)');
...
function upload2() {
    ...
    // 文件上传过程中创建进度条实时显示。
    uploader.on('uploadProgress', function (file) {
        $('#img2').after('<label class="loading"></label>');
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            $('#img2').siblings('.loading').remove();
            $('#img2').show();
            layer.msg(ret.info);
        } else {
            $('#qrcode').val(ret.data.url);
            $("#img2").attr('onload', 'setImgWH(this)');
...
function upload3() {
    ...
    // 文件上传过程中创建进度条实时显示。
    uploader.on('uploadProgress', function (file) {
        $('#img3').after('<label class="loading"></label>');
    ...
    uploader.on('uploadSuccess', function (file, ret) {
        if (ret.status == 0) {
            $('#img3').siblings('.loading').remove();
            $('#img3').show();
            layer.msg(ret.info);
        } else {
            $('#open_qrcode').val(ret.data.url);
            $("#img3").attr('onload', 'setImgWH(this)');
...
function upload4() {
    ...
    // 文件上传过程中创建进度条实时显示。
    uploader.on('uploadProgress', function (file) {
        $('#' + file.id + ' .img4').after('<label class="loading"></label>');
    ...
    //删除图片
    $("#filelist4").on("click", ".close", function () {
        // 移除队列中文件
