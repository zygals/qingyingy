...
<body>
    <form enctype="multipart/form-data" id="PostMe" method="post" name="upform">
        ...
        <div class="uploadtext">上传</div>
			<div class="inputfile" title="点击上传图片">
				<input type="file" name="img" id="uploadicon" value="upload" accept="image/jpeg,image/jpg,image/png,image/gif"/>
			</div>
		</div>
    ...
    <script type="text/javascript">
		...
		<?php if($Img == ''): ?>$("#ImgPr").attr('src',$('#<?php echo ($BackCall); ?>',parent.document).val());
		<?php else: ?>
