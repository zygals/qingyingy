...
class ArticleController extends ComController
{
    ...
    public function update($aid = 0)
    {
        ...
        $data['befrom'] = I('post.befrom', '', 'trim');
        $data['seotitle'] = I('post.seotitle', '', 'trim');
        $data['keywords'] = I('post.keywords', '', 'trim');
        ...
        if ($aid) {
            ...
            addlog('编辑文章，AID：' . $aid);
            $this->success('恭喜！文章编辑成功！',U('article/index'),1);
        } else {
