...
class EvaController extends ComController{
    ...
    public function index(){
		...
		$count      = $m->cache('meva_list_list_count',3600)->where($where)->count();
		$Page       = new \Think\Page($count,8);
		$list = $m->cache('meva_lists_list_'.$p,3600)->where($where)->order('aid desc')->limit($Page->firstRow.','.$Page->listRows)->select();
