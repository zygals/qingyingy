...
class NewsController extends ComController{
	private function getcate(){
		$cates=M('category')->cache('getcate',3600)->order('id asc')->select();
    ...
    public function index(){
		...
		$p=I('get.p','0','intval');
		$list = $m->cache('news_index_list_'.$p,3600)->where($where)->order('aid desc')->page($_GET['p'].',12')->select();
		$this->assign('list',$list);
		$count      = $m->cache('news_index_list_count',3600)->where($where)->count();
    ...
    public function lists(){
		...
		$m=M('article');
		$list = $m->cache('news_lists_list_'.$p.$dir,3600)->where("sid='{$cate['id']}' and status=1")->order('aid desc')->page($_GET['p'].',10')->select();
		
		$this->assign('list',$list);
		$count      = $m->cache('news_list_list_count'.$p.$dir,3600)->where("sid='{$cate['id']}' and status=1")->count();
